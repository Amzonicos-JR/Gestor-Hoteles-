...
export const GetInvoiceDetailEvents = () => {
  ...
  const getEventsNot = async()=>{
    try{
        const { data } = await axios.get(`http://localhost:3000/invoicedetail/geteventsnot/${_id}`)
        if(data.eventsNot){
        ...
        throw new Error(
          err.response.message ||
            "Error getting events not on invoice detail"
        );        
    }
  }
  ...
  const addEvent = async(idEvent)=>{
    try{
        ...
        const { data } = await axios.put(`http://localhost:3000/invoicedetail/addevent/${_id}`, form)
        if(data.message){
            getEvents();
            getEventsNot();
        }
  ...
  const deleteEvent = async(idEvent)=>{
    try{
        ...
        const { data } = await axios.put(`http://localhost:3000/invoicedetail/deleteevent/${_id}`, form)
        if(data.message){
            getEventsNot()
            getEvents()
        }
